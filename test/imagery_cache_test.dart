...
const _a = TileId(17, 129167, 79983);
const _b = TileId(17, 129168, 79983);
...
Future<ImageryCache> _aged(Duration age) async {
  ...
  return ImageryCache.open(_work);
}
...
void main() {
  ...
  test('holds nothing to begin with', () async {
    final cache = await ImageryCache.open(_work);
  ...
  test('is still there after opening again', () async {
    final first = await ImageryCache.open(_work);
    await first.write(_a, _bytes(512));
    final second = await ImageryCache.open(_work);
  ...
  test('counts a tile as current for its first week', () async {
    ...
    final older = await _aged(imageryFreshness * 2);
  ...
  test('throws away the oldest when it runs out of room', () async {
    final cache = await ImageryCache.open(_work, maximumBytes: 1);
    await cache.write(_a, _bytes(64));
  ...
  test('stays inside its limit', () async {
    final cache = await ImageryCache.open(_work, maximumBytes: 4096);
    for (var i = 0; i < 20; i++) {
      await cache.write(TileId(17, 129167 + i, 79983), _bytes(512));
    }
  ...
  test('starts again rather than trusting an index it cannot read', () async {
    ...
    expect((await ImageryCache.open(_work)).tiles, isEmpty);
  });
