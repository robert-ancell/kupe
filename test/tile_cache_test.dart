...
const _a = TileId(16, 60000, 40000);
const _b = TileId(16, 60001, 40000);
...
void main() {
  ...
  test('holds nothing to begin with', () async {
    final cache = await TileCache.open(_work);
  ...
  test('is still there after opening again', () async {
    final first = await TileCache.open(_work);
    await first.write(_a, _tile(1));
    final bytes = first.bytes;

    final second = await TileCache.open(_work);
  ...
  test('remembers where the map was left', () async {
    ...
    final second = await TileCache.open(_work);
    expect(second.camera, isNotNull);
    expect(second.camera!.latitude, closeTo(-36.85, 1e-6));
    expect(second.camera!.longitude, closeTo(174.76, 1e-6));
    expect(second.camera!.zoom, 17);
  });
  ...
  test('throws away the oldest when it runs out of room', () async {
    final cache = await TileCache.open(_work, maximumBytes: 1);
    await cache.write(_a, _tile(1));
  ...
  test('stays inside its limit', () async {
    final cache = await TileCache.open(_work, maximumBytes: 4000);
    for (var i = 0; i < 20; i++) {
      await cache.write(TileId(16, 60000 + i, 40000), _tile(i * 100));
    }
